...
1. **Connexion** — le bouton "Se connecter avec Twitch" reste LE choix mis en avant (seul chemin
   qui débloque présence live + tirages premium, §5bis). Depuis le 23/07/2026, un second chemin
   existe pour un viewer qui ne veut pas connecter Twitch tout de suite : email + mot de passe
   (Supabase Auth), avec possibilité d'**associer Twitch après coup** depuis un bandeau sur
   l'accueil — rien n'est perdu à choisir ce chemin d'abord.
...
4. **⚔️ Combat** (plein écran, arène en **3/4 portrait**, pas 16/9 — voir §7). Trois moments :
   **① l'écran VS** — portraits qui entrent de chaque côté avec dépassement et 150 ms de décalage, "VS" qui claque au centre (scale 3→1 + flash blanc + micro-secousse), fond coupé en diagonale aux couleurs de classe des deux combattants, **niveau + rareté** affichés (c'est l'enjeu réel : « Arlong Niv 3 vs Crocodile Niv 1 »), **« le pirate de {pseudo} »** sous le portrait adverse, et un bandeau de matchup **toujours affiché** *(changé le 23/07/2026 — avant, le bandeau disparaissait entièrement en l'absence d'avantage, impossible de vérifier que c'était bien neutre)* : **« ⚔️ Haki vs Paramecia — avantage Haki »**, ou **« — neutre »** si aucune classe ne contre l'autre. Durée : 3,8 s, calée sur le **temps de lecture** du bandeau et non sur celle des animations. Tap pour lancer plus tôt.
